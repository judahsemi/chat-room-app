...
class LoggedOutUserTests(BaseTests):
    ...
    def test_lounge(self):
        resp = self.unlogged_client.get(url_for("room_bp.lounge", room=self.room))
        self.assertEqual(resp.status_code, 302)
        self.assertIn(url_for("room_bp.login"), resp.location)

    def test_bad_url_build(self):
        with self.assertRaises(Exception):
            url_for("room_bp.lounge", room=self.room.id)
        with self.assertRaises(Exception):
            url_for("room_bp.lounge", room=self.user)
...
class LoggedInUserTests(BaseTests):
    ...
    def test_lounge(self):
        resp = self.client.get(url_for("room_bp.lounge", room=self.room))
        self.assertEqual(resp.status_code, 200)
        self.assertIn("Enter", self.get_text(resp))
        self.assertIn(str(self.room.number), self.get_text(resp))
        self.assertIn("form", self.get_text(resp))
        self.assertIn(url_for("room_bp.dashboard", _external=False),
            self.get_text(resp))

    def test_lounge_post(self):
        resp = self.client.post(url_for("room_bp.lounge", room=self.room),
            data=self.first_time_info[1])
        self.assertEqual(resp.status_code, 302)
        self.assertIn(url_for("room_bp.room", room=self.room), resp.location)

    def test_lounge_not_joined(self):
        resp = self.another_client.get(url_for("room_bp.lounge", room=self.room),
